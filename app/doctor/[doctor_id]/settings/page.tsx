...
export default function Page({ params }: pageProps) {
  ...
  const uploadProfile = async () => {
    if (file) {
      ...
      formdata.append('ref', 'api::doctor.doctor');
      formdata.append('refId', doctorID);
  ...
  useEffect(() => {
    if (isUuid(params.doctor_id)) {
      findOneDoctor(params.doctor_id).then((data) => {
        if (data.doctors.data.length === 0) {
          showToastMessage('error', 'Invalid Doctor ID');
          setTimeout(() => {
            router.push('/login');
          }, 2000);
        } else {
          ...
          setAddress(data.doctors.data[0].attributes.address);
